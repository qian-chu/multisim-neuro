...
def test_simulator_init_minimal():
    ...
    effects = [{"condition": "cond", "windows": [[0.1, 0.3]], "effect_size": 1.0}]
    sim = Simulator(
        X=X,
        noise_std=0.1,
        n_channels=4,
...
def test_invalid_tmax_tmin():
    ...
    effects = [{"condition": "cond", "windows": [[0.1, 0.2]], "effect_size": 1.0}]
...
def test_invalid_effects_missing_condition():
    X = pd.DataFrame(np.random.randn(5, 1), columns=["cond"])
    effects = [{"windows": [[0.1, 0.2]], "effect_size": 1.0}]
...
def test_malformed_window():
    X = pd.DataFrame(np.random.randn(5, 1), columns=["cond"])
    effects = [{"condition": "cond", "windows": [], "effect_size": 1.0}]
...
def test_invalid_effects_missing_amp_and_size():
    X = pd.DataFrame(np.random.randn(5, 1), columns=["cond"])
    effects = [{"condition": "cond", "windows": [[0.1, 0.2]]}]
